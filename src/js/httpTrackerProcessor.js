let eventTracker = (function () {
  ...
  const COOKIE_CONTENT_BANNER_OPTIMIZED =
    "<tr><td colspan=2 class='web_event_detail_cookie'>Cookies (optimized)</td></tr>";
  const COOKIE_CONTENT_BANNER_UNOPTIMIZED =
    "<tr><td colspan=2 class='web_event_detail_cookie'>Cookies (unoptimized)</td></tr>";
  const ignoreHeaders = [
    'frameAncestors',
    'frameId',
    'parentFrameId',
  ...
  function actionOnBeforeRedirect(webEvent) {
    if (webEvent.callerName === 'onBeforeRedirect') {
      ...
      // }
      if (webEvent.ip) {
        // webEvent.ip && webEvent.statusCode && webEvent.statusLine && webEvent.redirectUrl
  ...
  function isEventToCapture(webEvent) {
    ...
    let captureEventExclude = captureEventInclude
      ? urlMatchExcludePattern(webEvent)
      : true;
  ...
  function urlMatchIncludePattern(webEvent) {
    ...
    if (includeURLsList && includeURLsList.length) {
      found = includeURLsList.some((v) =>
        webEvent.url.toLowerCase().includes(v)
      );
    }
    if (!found && globalIncludeURLsList && globalIncludeURLsList.length) {
      found = globalIncludeURLsList.some((v) =>
        webEvent.url.toLowerCase().includes(v)
      );
    }
    {
  ...
  function urlMatchExcludePattern(webEvent) {
    let toExclude = false;
    if (excludeURLsList) {
      toExclude = excludeURLsList.some((v) =>
        webEvent.url.toLowerCase().includes(v.toLowerCase())
      );
    } else if (!toExclude && globalExcludeURLsList) {
      toExclude = globalExcludeURLsList.some((v) =>
        webEvent.url.toLowerCase().includes(v.toLowerCase())
      );
    }
    return toExclude;
  ...
  function maskFieldsPattern(value) {
    ...
    if (maskAttributesCheckboxValue) {
      if (maskedAttributesList) {
        masking = maskedAttributesList.some((v) =>
          value.toLowerCase().includes(v)
        );
      }
      if (!masking && globalMaskPatternsList) {
        masking = globalMaskPatternsList.some((v) =>
          value.toLowerCase().includes(v)
        );
      }
    }
  ...
  function addEventList(webEvent) {
    ...
    let containerContent =
      "<div title='Click to view details' class='" +
      CLASS_LIST_TO_ADD +
      "' id='web_events_list_" +
      webEvent.requestIdEnhanced +
      "'>" +
  ...
  function updateEventList(webEvent) {
    if (webEvent.callerName === 'onErrorOccurred') {
      // onErrorOccurred contains webEvent.error not webEvent.statusCode
      getById(`web_events_list_${webEvent.requestIdEnhanced}`).classList.add(
        'web_event_style_error'
      );
      getById(`web_event_status_${webEvent.requestIdEnhanced}`).innerHTML =
        STRING_ERROR;
    }
    // do not update if statusCode is not available (ex: service workers, fetch events in FF are missing response events)
      ...
      getById(`web_events_list_${webEvent.requestIdEnhanced}`).classList.remove(
        'web_event_style_error'
      );
      getById(`web_event_status_${webEvent.requestIdEnhanced}`).innerHTML =
        webEvent.statusCode;
    }
    getById(`web_event_cache_${webEvent.requestIdEnhanced}`).innerHTML =
      webEvent.fromCache ? webEvent.fromCache : 'N/A';
  }
  ...
  function filterEventList(webEvent) {
    if (multipleSearchPatterns.length) {
      ...
      if (type === 'DATE') {
        value = `${
          webEvent.timeStamp ? getReadableDate(webEvent.timeStamp) : 'N/A'
        }`;
      }
      if (!value.toString().toLowerCase().includes(filterWithValue)) {
        ...
        getById(
          `web_events_list_${webEvent.requestIdEnhanced}`
        ).classList.remove('web_event_list_hide');
      }
    }
  }
  ...
  function generateSTATUSContent(webEvent) {
    return `<div class='web_event_list_status' id='web_event_status_${
      webEvent.requestIdEnhanced
    }'>${
      webEvent.statusCode
        ? webEvent.statusCode
  ...
  function generateDATETIMEContent(webEvent) {
    return `<div class='web_event_list_date_time' id='web_event_time_${
      webEvent.requestIdEnhanced
    }'>${
      webEvent.timeStamp ? getReadableDate(webEvent.timeStamp) : 'N/A'
    }</div>`;
  ...
  function displaySelectedEventDetails(webEvent) {
    if (
      ...
      webEvent.requestIdEnhanced === selectedWebEventRequestId
    ) {
  ...
  function displayEventProperties() {
    ...
    let requestContainer = buildURLDetailsContainer(
      webEventIdRequest,
      'requestDetails'
    );
    let responseContainer = buildURLDetailsContainer(
      webEventIdResponse,
      'responseDetails'
    );
    let requestFormContainer = buildRequestFormContainer(webEventIdRequestForm);
    getById('web_event_details_selected_request').style.borderRight =
      '1px solid';
    getById('web_event_details_selected_response').style.borderRight =
      '1px solid';
    getById('web_event_details_selected_request').style.borderLeft =
      '1px solid';
    getById('web_event_details_selected_response').style.borderLeft =
      '1px solid';
    getById('web_event_details_selected_request').style.borderBottom =
      '1px solid';
    getById('web_event_details_selected_response').style.borderBottom =
      '1px solid';
    getById('request_headers_details').innerHTML =
      requestContainer + requestFormContainer;
  ...
  function buildURLDetailsContainer(webEventIdDetails, detailsType) {
    ...
    if (webEventIdDetails) {
      ...
      Object.entries(webEventIdDetails).forEach(([key, value]) => {
        if (key !== 'responseHeaders' && key !== 'requestHeaders') {
          // headers added by browser
          if (
            !ignoreHeaders.includes(key) &&
            value !== undefined &&
            value !== null
          ) {
  ...
  function generateHeaderKeyValueContent(key, value) {
    ...
    return `${HEADER_CONTENT_KEY}${addMarkTag(
      key
    )}${HEADER_CONTENT_VALUE}${addMarkTag(value)}`;
  }
  ...
  function addMarkTag(text) {
    if (findPatterns.length) {
      ...
      let markedText = text
        .toString()
        .replace(findStr, (match) => `<mark>${match}</mark>`);
      return markedText;
    }
    return text;
  }
  ...
  function buildRequestFormContainer(webEventIdRequestForm) {
    ...
    if (webEventIdRequestForm) {
      if (webEventIdRequestForm.formData) {
        formData =
          "<tr><td colspan=2 class='web_event_detail_cookie'>Body (form fields data)</td></tr>";
        Object.entries(webEventIdRequestForm.formData).forEach(
          ([key, value]) => {
        ...
        formData =
          "<tr><td colspan=2 class='web_event_detail_cookie'>Body (raw form data)</td></tr>";
        for (let eachByte of webEventIdRequestForm.raw) {
          ...
          formData += `<tr style='white-space: pre-wrap; word-break: break-all;'><td colspan=2>${addMarkTag(
            decodedString
          )}</td></tr>`;
        }
      }
  ...
  function generateHeaderDetails(headers) {
    ...
    headers.forEach((header) => {
      ...
      if (header.name === DELIMITER_REQUEST_COOKIE_KEY_NAME) {
        cookieContent += generateRequestCookieDetails(
          header.value,
          DELIMITER_REQUEST_COOKIE
        );
      }
      // response cookies
      else if (
        header.name.toLowerCase() === DELIMITER_RESPONSE_COOKIE_KEY_NAME
      ) {
        if (!optimizeResponseCookies) {
          banner = COOKIE_CONTENT_BANNER_UNOPTIMIZED;
          cookieContent += generateResponseCookieDetails(
            header.value,
            DELIMITER_RESPONSE_COOKIE
          );
        } else {
          banner = COOKIE_CONTENT_BANNER_OPTIMIZED;
          setOptimizedCookiesMap(
            header.value,
            DELIMITER_RESPONSE_COOKIE,
            optimizedCookiesMap
          );
      ...
      // other headers
        ...
        generalHeadersContent += `${HEADER_CONTENT_KEY}${addMarkTag(
          header.name
        )}${HEADER_CONTENT_VALUE}${addMarkTag(header.value)}</td></tr>`;
      }
    });
    if (optimizeResponseCookies) {
      sortMapByKey(optimizedCookiesMap).forEach((value, key) => {
        cookieContent += `${HEADER_CONTENT_KEY}${addMarkTag(
          key.split(':', 1)
        )}${HEADER_CONTENT_VALUE}${addMarkTag(value.cookieValue)}</td></tr>`;
      });
    }
    if (cookieContent) {
  ...
  function generateRequestCookieDetails(cookieValue, cookieDelim) {
    ...
    cookieList.forEach((cookie) => {
      if (cookie) {
        ...
        if (firstOccurance > -1) {
          cookieMap.set(
            cookie.substring(0, firstOccurance),
            cookie.substring(firstOccurance + 1)
          );
        }
      }
    });
    cookieMap.forEach((value, key) => {
      cookieContent += `${HEADER_CONTENT_KEY}${addMarkTag(
        key
      )}${HEADER_CONTENT_VALUE}${addMarkTag(value)}</td></tr>`;
    });
  ...
  function generateResponseCookieDetails(cookieValue, cookieDelim) {
    ...
    cookieList.forEach((cookie) => {
      if (cookie) {
        ...
        cookieContent += `${HEADER_CONTENT_KEY}${addMarkTag(
          cookieDetails.cookieName
        )}${HEADER_CONTENT_VALUE}${addMarkTag(
          cookieDetails.cookieValue
        )}</td></tr>`;
      }
    });
    return cookieContent;
  ...
  function setOptimizedCookiesMap(
    cookieValue,
    cookieDelim,
    optimizedCookiesMap
  ) {
      ...
      if (cookieObj.cookieValue) {
        stringToArray(cookieObj.cookieValue, ';').forEach((attribute) => {
          if (attribute) {
            ...
            if (
              attributeKeyValue[0].toLowerCase() === 'domain' ||
              attributeKeyValue[0].toLowerCase() === 'path'
            ) {
              cookieObj[attributeKeyValue[0].toLowerCase()] =
                attributeKeyValue[1];
            }
  ...
  function getHiddenUrlsList() {
    return getById('urls_list').getElementsByClassName(
      'web_event_list_blank web_event_list_hide'
    ); // this returns a live collection
  }
  ...
  function bindDefaultEvents() {
    ...
    getById('optimize_response_cookies').onchange =
      optimizeResponseCookiesCheckbox;
    getById('filter_web_events').oninput = filterEvents;
    getById('web_event_filter_key').oninput = filterEvents;
    getById('clear_filter_web_events').onclick =
  ...
  function generateHeadersToAddOrModify() {
    ...
    Array.prototype.filter.call(conatiners, function (headerContainer) {
      ...
      headerName = headerContainer
        .querySelector('.header_input_name')
        .value.trim();
      if (headerName) {
        if (
          !FORBIDDEN_HEADERS.some(
            (v) => headerName.toLowerCase() === v.toLowerCase()
          ) &&
          !FORBIDDEN_HEADERS_PATTERN.some((v) =>
            headerName.toLowerCase().startsWith(v.toLowerCase())
          )
        ) {
          ...
          if (headerContainer.querySelector('.header_input_apply').checked) {
            let x = {};
            x.name = headerName;
            x.value = headerContainer.querySelector(
              '.header_input_value'
            ).value;
            x.url = headerContainer
              .querySelector('.header_input_url')
              .value.trim();
    ...
    if (headersObject.length || conatiners.length) {
      getById(
        'add_modify_headers_banner'
      ).innerHTML = `Add/Modify request headers: ${headersObject.length}`;
    } else {
      getById(
        'add_modify_headers_banner'
      ).innerHTML = `Add/Modify request headers:`;
    }
  }
  ...
  function updateSelectedEventToContainer(event) {
    if (
      event.target &&
      event.target.classList.contains('web_event_list_container')
    ) {
  ...
  function selectNextEligibleEvent(selectedEvent, keyCode) {
    if (keyCode == 40) {
      // down arrow
      let nextElement = selectedEvent
        ? selectedEvent.nextElementSibling
        : selectedEvent;
      while (nextElement) {
      ...
      // up arrow
      let previousElement = selectedEvent
        ? selectedEvent.previousElementSibling
        : selectedEvent;
  ...
  function setEventRowAsSelected(event) {
    if (
      event.target &&
      event.target.parentElement.classList.contains('web_event_list_blank')
    ) {
      markSelectedRequest(event.target.parentElement.id);
      getById('delete_selected_web_event').disabled = false;
      getById('delete_selected_web_event').classList.remove(
        'web_event_list_filtered'
      );
    }
  }
  ...
  function updateFilterOptions() {
    ...
    if (filterWithValue) {
      if (
        filterWithValue.length < 3 ||
        (filterWithValue.includes(DELIMITER_OR) &&
          filterWithValue.includes(DELIMITER_AND))
      ) {
        // invalid search filter text, do nothing
      } else {
        if (filterWithValue.includes(DELIMITER_AND)) {
          multipleSearchPatterns = stringToArray(
            filterWithValue,
            DELIMITER_AND
          );
  ...
  function getGlobalOptions(details) {
    globalExcludeURLsList = getPropertyFromStorage(
      details,
      httpTracker.STORAGE_KEY_EXCLUDE_PATTERN
    );
    globalMaskPatternsList = getPropertyFromStorage(
      details,
      httpTracker.STORAGE_KEY_MASK_PATTERN
    );
    globalIncludeURLsList = getPropertyFromStorage(
      details,
